...
interface FlickrApi {
    @GET("/services/rest/?method=flickr.photos.search" +
            ...
            "&format=json" +
            "&nojsoncallback=1")
    fun getSearchResults(@Query("api_key") key: String, @Query("text") searchText: String): Observable<FlickrResponse>

    @GET("services/rest/?method=flickr.photos.getInfo" +
            "&format=json" +
            "&nojsoncallback=1")
    fun getPhotoInfo(@Query("photo_id") photoId: String, @Query("api_key") key: String): Observable<FlickrResponse>
